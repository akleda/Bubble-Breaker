...
/**
 * Class for creating field of colors represented by integers 1 -
 * NUMBER_OF_COLORS. The class allows to search in the field for other positions
 * arund a specified one which contain the same color. It also does some other
...
public class Bubbles {
    ...
    /**
     * Populate field with random colors (represendted by integers 1 -
     * NUMBER_OF_COLORS)
     */
    ...
    /**
     * Recursive method helping to move the bubbles down in case there is a
     * space between them.
     *
     * @param i
     * @param j
    ...
    /**
     * Recursive method for helping move the bubbles right in case there is a
     * space between them
     *
     * @param i
     * @param j
    ...
    private void search(int i, int j, int selectedColor) {
        ...
        } else if (selectedColor == getBubbleField()[i][j]) {
    ...
    /**
     * Method which returns the value of the selected bubble to be used in the
     * score
     *
     * @param i
     * @param j
    ...
    /**
     * Method which calculates lines arround selected bubble
     *
     * @param x
     * @param y
     * @return
    ...
    public ArrayList<Line> linesToDraw(int x, int y) {
        ...
        for (Position p : sameColorAround) {
            if (!hasNeighbourUp(p.x, p.y)) {
                lines.add(new Line(p.x, p.y, p.x + 1, p.y));
            }
            if (!hasNeighbourDown(p.x, p.y)) {
                lines.add(new Line(p.x, p.y + 1, p.x + 1, p.y + 1));
            }
            if (!hasNeighbourLeft(p.x, p.y)) {
                lines.add(new Line(p.x, p.y, p.x, p.y + 1));
            }
            if (!hasNeighbourRight(p.x, p.y)) {
                lines.add(new Line(p.x + 1, p.y, p.x + 1, p.y + 1));
            }
        }
        return lines;
    }
    ...
    /*
     *  Method which helps to find out where the lines around the selected bubble are  
     */
